...
Defines the MarkdownIndex generated from canonical source, including headings, blocks, source ranges, and raw island detection.

---

## 2. Motivation

- Source-preserving visual editing requires reliable mapping from projections back to source.
- The index is the foundation for outlines, raw islands, and patches.

---

## 3. Goals

- Parse canonical Markdown into a block-oriented index.
- Record Rust-owned UTF-8 byte ranges for safe regions.
- Expose heading outline and block projections.
- Identify unsupported or risky regions as Raw Markdown Islands.
- Support full reparse after each mutation for MVP.

---

## 4. Non-Goals

- Build a perfect Markdown AST for every extension.
- Implement incremental parsing in MVP.
- Trust JS-generated byte offsets.
- Rewrite unsupported Markdown into normalized Markdown.
...
- The user sees outline headings and Form Mode blocks derived from this index.
- Unsafe regions are shown as raw islands rather than disappearing or being rewritten.
...
struct BlockNode {
    ...
    content_range: Option<ByteRange>,
    fingerprint: BlockFingerprint,
    editable_policy: EditablePolicy,
}

struct ByteRange { start: usize, end: usize }
```

ByteRange values are Rust UTF-8 byte offsets into `canonical_text`. They are never authoritative when supplied by JS.

---

## 8. Internal Design Notes

- Use parser offset information where available.
- For block grouping not directly produced by parser events, build a conservative source scanner.
- Prefer false negatives over unsafe false positives: mark a region as raw island if uncertain.
- Unit-test range mapping with Japanese text, emoji, CRLF, front matter, HTML, lists, and code fences.
...
- The index identifies headings for outline display.
- The index maps safe blocks to byte ranges.
- Unsupported regions are represented as raw islands.
- Reparsing after mutation produces a valid new index.
