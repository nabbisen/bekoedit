# RFC-040: Sync and Collaboration Evaluation
...
Evaluates cloud sync and collaboration while reaffirming that they are non-goals for MVP.

---

## 2. Motivation

- Users may ask for sync and collaboration, but these features can dominate architecture.
- Local-first editing should remain independent.

---

## 3. Goals

- Document why sync/collaboration are excluded from MVP.
- Identify what would need to change if reconsidered.
- Preserve compatibility with third-party sync tools through safe file conflict behavior.

---

## 4. Non-Goals

- Implement proprietary cloud sync.
...
- MVP documentation recommends external sync tools with conflict caution.
...
```rust
struct SyncEvaluation { required_identity: bool, required_remote_storage: bool, conflict_model: String }
```
...
- Current conflict detection should remain useful even when files are changed by external sync clients.
...
- Future sync decision does not affect MVP local-first release.
