# RFC-004: Native File Explorer and File Tree Index
...
Defines the workspace file tree, Markdown filtering, ignore rules, sorting, selection, and tree projection used by the sidebar explorer.

---

## 2. Motivation

- The file explorer is a core value proposition.
- Large folders require predictable indexing rules and responsive tree rendering.
- The tree must show Markdown files without exposing too much filesystem noise.

---

## 3. Goals

- Display workspace directories and Markdown files.
- Support `.md` and `.markdown` by default.
- Ignore high-noise directories such as `.git`, `node_modules`, and `target`.
- Keep current selection stable across refreshes when possible.
- Expose accessible tree navigation.
...
- Display binary assets as first-class items in MVP.
- Implement Git status in the tree.
- Implement multi-root tree display.
...
- Left sidebar contains workspace tree.
- Directories can be expanded/collapsed.
- The selected file is visually distinct and keyboard-focusable.
- File tree loading and refresh state are visible.
...
enum FileNodeKind { Directory, MarkdownFile, UnsupportedFilePlaceholder }
```

The UI receives a projection, not raw filesystem handles.

---

## 8. Internal Design Notes

- Build the tree in Rust using safe filesystem traversal.
- Protect against symlink loops and permission-denied directories.
- Represent inaccessible directories as non-fatal tree warnings.
- Use stable file IDs based on workspace-relative paths plus revision-aware mapping for rename handling.
...
- A workspace with nested Markdown files renders as a navigable tree.
- Ignored directories are hidden by default.
- Permission errors are visible but do not break the whole tree.
- Selection persists across refresh when the selected file still exists.
