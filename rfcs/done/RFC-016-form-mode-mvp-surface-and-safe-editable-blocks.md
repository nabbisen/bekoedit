# RFC-016: Form Mode MVP Surface and Safe Editable Blocks
...
Defines the initial visual Form Mode, including editable block types, layout, commands, and safe downgrade behavior.

---

## 2. Motivation

- Visual editing is a core product promise, but arbitrary WYSIWYG is unsafe.
- MVP Form Mode should edit only structures that can be patched safely.

---

## 3. Goals

- Provide visual block editing for headings, paragraphs, simple lists, task lists, blockquotes, code blocks, horizontal rules, links, emphasis, and images as lightweight blocks.
- Represent unsupported regions as Raw Markdown Islands.
- Send semantic edit commands to Rust.
- Keep Text Mode available as escape hatch.

---

## 4. Non-Goals

- Implement complex table visual editing.
- Implement arbitrary inline HTML editing visually.
- Use rich-text AST as persistence format.
...
```text
+--------------------------------------------------------------+
| file.md   Saved                         [Text] [Form] [Preview]|
+--------------------------------------------------------------+
| # Title                                                      |
|                                                              |
| Paragraph text appears visually editable.                    |
|                                                              |
| - list item one                                              |
| - list item two                                              |
|                                                              |
| [Raw Markdown Island: HTML block] [Edit raw]                 |
...
Each block has subtle focus and action affordances but should not feel like a heavy block editor.
...
struct FormProjection {
    ...
    blocks: Vec<FormBlock>,
    warnings: Vec<FormWarning>,
}
```

FormProjection is rebuilt from MarkdownIndex. It is not durable.

---

## 8. Internal Design Notes

- Start with simple HTML controls/contenteditable wrappers only where patch generation is safe.
- Keep a per-block toolbar minimal.
- For MVP, avoid nested complex list transformations unless source preservation tests exist.
- Every Form Mode edit command must be accepted or rejected by Rust validation.
...
- User can edit safe block text visually.
- Unsupported blocks appear as raw islands.
- Switching Form -> Text shows preserved Markdown source.
- Editing a Form block does not rewrite unrelated source.
