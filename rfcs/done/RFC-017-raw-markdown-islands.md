...
Defines Raw Markdown Islands as safe representations for unsupported, risky, or user-authored Markdown regions that should not be visually transformed.

---

## 2. Motivation

- Raw islands prevent silent corruption.
- They allow Form Mode to exist without pretending to support every Markdown extension.

---

## 3. Goals

- Detect front matter, HTML blocks, complex tables, math blocks, directives, malformed regions, and unknown extensions as raw islands when needed.
- Provide raw editing inside Form Mode where safe.
- Keep raw islands visually distinct and accessible.
- Avoid altering island content unless user edits it directly.

---

## 4. Non-Goals

- Visually edit every island type in MVP.
- Validate every possible extension syntax.
- Hide unsupported Markdown from users.
- Convert raw islands into normalized Markdown.
...
- Raw island presentation:

```text
+--------------------------------------------------+
| Raw Markdown Island: YAML front matter           |
| This region is preserved as source.              |
| [Edit raw] [Open in Text Mode]                   |
| ---                                              |
| title: Example                                   |
| tags: [draft, note]                              |
| ---                                              |
...
Read-only islands can still offer “Open in Text Mode”. Editable islands use a small raw text area.
...
```rust
struct RawIsland {
    island_id: RawIslandId,
    source_range: ByteRange,
    island_type: RawIslandType,
    edit_policy: RawIslandEditPolicy,
    reason: String,
}

enum RawIslandType { FrontMatter, HtmlBlock, ComplexTable, MathBlock, Directive, UnknownExtension, MalformedRegion }
enum RawIslandEditPolicy { RawEditable, ReadOnlyWithTextModeEscape }
```

Raw island ranges are Rust-owned and revision-scoped.
...
- Raw island edits use SourcePatch through Rust, not direct DOM persistence.
- When an edited island becomes parseable as a safe block, it may remain island until next full projection refresh.
- Use diagnostics to help users understand why an island exists.
...
- Front matter is preserved and not visually rewritten.
- HTML blocks are not executed as active scripts.
- Raw island direct edits patch only the island range.
- Unsupported syntax remains visible in Form Mode.
