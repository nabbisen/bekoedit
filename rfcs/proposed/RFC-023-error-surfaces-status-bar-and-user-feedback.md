# RFC-023: Error Surfaces, Status Bar, and User Feedback
...
Defines consistent user feedback for save state, file operations, parser warnings, raw islands, bridge errors, and conflicts.

---

## 2. Motivation

- A source-preserving editor must make safety states visible.
- Silent errors are unacceptable for file editing tools.

---

## 3. Goals

- Define status bar fields.
- Define banners, inline warnings, dialogs, and notification toasts.
- Make errors actionable and non-destructive.
- Route technical errors into user-facing messages.
...
- Create a complex notification center in MVP.
- Hide safety warnings for aesthetics.
...
- Status bar shows save state, active workspace, watcher state, line/column where applicable, and latest important warning.
- Conflict banners appear above editor surface.
- Raw island warnings appear inline.
- File operation errors appear near the explorer and in status.
...
Errors should preserve enough internal code for bug reports without overwhelming users.

---

## 8. Internal Design Notes

- Create error conversion layers from core errors to user-facing errors.
- Never panic for expected filesystem or parsing failures.
- Use assertive accessibility announcements for critical data-safety errors.
...
- Conflict state cannot be missed.
- Parser warnings do not block raw editing.
- Bridge/deserialization errors do not crash the app.
