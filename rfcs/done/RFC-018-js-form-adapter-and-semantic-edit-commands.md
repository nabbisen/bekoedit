# RFC-018: JS Form Adapter and Semantic Edit Commands
...
Defines the browser-side Form Mode adapter and the semantic command payloads sent to Rust.

---

## 2. Motivation

- The visual editor needs to communicate user intent without owning source ranges.
- Semantic commands allow Rust to preserve Markdown style.

---

## 3. Goals

- Define Form Mode commands by semantic intent.
- Avoid authoritative byte ranges in JS.
- Include document revision and block identity validation metadata.
- Keep adapter replaceable if Lexical or another engine is introduced later.

---

## 4. Non-Goals

- Commit to Lexical for MVP.
- Expose Rust patch internals to JS.
- Let JS serialize full Markdown for normal Form Mode saves.
- Implement arbitrary rich-text transformations.
...
- The user sees immediate visual editing feedback.
- If a command is rejected due to staleness, the block refreshes with minimal disruption.
...
enum FormBlockEdit {
    ...
    SetHeadingLevel { level: u8 },
    ToggleTaskChecked { item_ordinal: u32, checked: bool },
    ReplaceListItemText { item_ordinal: u32, text: String },
    SetLinkTarget { inline_id: InlineNodeId, href: String, title: Option<String> },
    ReplaceCodeBlock { language: Option<String>, code: String },
    ReplaceRawIsland { text: String },
    DeleteBlock,
}

struct FormEditCommand {
    document_id: DocumentId,
    base_revision: u64,
    block_id: BlockId,
    block_revision: u64,
    edit: FormBlockEdit,
    client_block_fingerprint: Option<BlockFingerprint>,
}
```

The absence of authoritative range fields is intentional.

**Review resolution (2026-06-07).** `ReplaceListItemText` and `DeleteBlock` were missing from the original draft but are required by the external design (§23.11 `FormBlockEdit`) and by RFC-016's safe block matrix (list item editing; horizontal rule delete action). They are added here. Task and list item targeting uses an item ordinal scoped to the parent block's revision, consistent with the revision-scoped `BlockId` model of RFC-014. `SetLinkTarget` remains specified but may ship after the other commands.

---

## 8. Internal Design Notes

- Implement adapter as a thin bridge around HTML controls or a small editor framework.
- Keep command schemas stable and versioned.
- Debounce high-frequency edits but preserve final user intent.
- Use acknowledgements from Rust to update block revision and status.
...
- Form adapter sends BlockId-based commands.
- Rust can reject stale commands.
- JS cannot force a patch at an arbitrary byte range.
- Adapter can be replaced without changing core document model.
