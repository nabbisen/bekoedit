# RFC-007: Save, Autosave, Atomic Write, and Recovery
...
Defines manual save, debounced autosave, atomic write strategy, crash recovery snapshots, and save-failure behavior.

---

## 2. Motivation

- Autosave is valuable only if it is safe.
- Writing directly to the target file on every keystroke risks corruption and poor user trust.

---

## 3. Goals

- Provide manual save and debounced autosave.
- Use atomic write where possible.
- Keep recovery snapshots for dirty sessions.
- Surface save failures clearly.
- Avoid data loss on crash or application termination.
...
- Implement multi-version document history.
- Implement Git commits.
- Guarantee atomic semantics on every filesystem beyond platform limitations.
...
- Status bar shows “Saving…”, “Saved”, “Save failed”, or “Recovered draft available”.
- Autosave can be enabled by default but must be visible.
- On startup after crash, recovery candidates are listed with restore/discard choices.
...
enum SaveReason { Manual, AutosaveDebounced, AppClosing, BeforeFileOperation }

struct RecoverySnapshot {
    document_id_hint: Option<DocumentId>,
...
Atomic save should follow: write temporary file, flush, optionally preserve permissions/metadata, then rename/replace.

---

## 8. Internal Design Notes

- Autosave debounce should be configurable internally and tested with deterministic timers.
- Do not run concurrent writes for the same document.
- Before writing, check for external modification when possible.
- Store recovery snapshots outside the workspace in app data.
- Remove recovery snapshots only after confirmed save.
...
- Manual save writes current canonical text to disk.
- Autosave triggers after a debounce interval.
- Save failure leaves dirty text intact in memory.
- A simulated crash before save can be recovered from a snapshot.
