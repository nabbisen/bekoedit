...
Defines logical block identity for Form Mode commands and rules for invalidating stale projections.

---

## 2. Motivation

- Byte ranges become stale after edits.
- The UI needs stable-enough references, but Rust must remain authoritative.
- Revision-scoped block identity prevents stale Form Mode edits from corrupting source.

---

## 3. Goals

- Define `BlockId` as revision-scoped logical identity.
- Include fingerprints and block revisions in Form Mode commands.
- Reject stale or mismatched commands safely.
- Refresh projections after accepted patches.

---

## 4. Non-Goals

- Make block IDs permanent across arbitrary document rewrites.
- Build a database of block identities.
- Allow JS to patch by byte range.
- Guarantee stable IDs through complex structural edits in MVP.
...
- If a visual block becomes stale due to another edit, the app should refresh it or show a brief “content changed, edit reapplied/please retry” message.
...
struct BlockFingerprint {
    ...
    prefix_context_hash: Option<Hash>,
    suffix_context_hash: Option<Hash>,
}
...
struct FormEditCommand {
    ...
    block_revision: u64,
    edit: FormBlockEdit,
    client_observed_range: Option<ByteRange>,
    client_block_fingerprint: Option<BlockFingerprint>,
}
```

`client_observed_range` is diagnostic only.

---

## 8. Internal Design Notes

- Regenerate BlockIds on full reparse.
- Allow simple remapping only when fingerprint and context clearly match.
- When uncertain, reject the command and request UI refresh.
- Keep stale-command rejection user-friendly.
...
- Form Mode commands target BlockId, not authoritative ByteRange.
- Stale revision commands are rejected or remapped only under safe conditions.
- A patch early in the document does not make later JS byte ranges authoritative.
- Tests cover multibyte text and block reorder scenarios.
