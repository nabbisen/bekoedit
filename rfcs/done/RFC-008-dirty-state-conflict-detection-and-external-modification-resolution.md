# RFC-008: Dirty State, Conflict Detection, and External Modification Resolution
...
Defines dirty-state transitions and user workflows for files modified, deleted, or renamed outside bekoedit.

---

## 2. Motivation

- Users often edit Markdown folders with Git, terminals, sync clients, and other editors.
- Blind autosave over external changes would destroy user data.

---

## 3. Goals

- Track clean, dirty, saving, save failed, externally modified, externally deleted, and conflicted states.
- Detect disk fingerprint changes before save.
- Provide clear resolution options.
- Avoid automatic overwrite when both app and disk changed.

---

## 4. Non-Goals

- Implement automatic semantic merge in MVP.
- Implement CRDTs.
- Resolve sync-provider conflicts automatically.
- Implement Git merge UI.
...
- If disk changed while local edits exist, show a conflict banner.
- Resolution options: reload from disk, keep local and save as overwrite, save local as copy, compare manually if available.
- If file is deleted externally, offer restore by saving, save as new file, or close without saving.
...
```rust
enum DocumentConflictState {
    None,
    ExternalModified { disk_fingerprint: FileFingerprint },
    ExternalDeleted,
    PathUnavailable,
    LocalDirtyAndExternalChanged,
}
...
Fingerprint checks may begin with metadata and use content hash only when needed.

---

## 8. Internal Design Notes

- Update base disk fingerprint after successful load/save.
- Watcher events should mark sessions as potentially stale; final check happens before write.
- Conflict resolution creates explicit user commands, not implicit background overwrites.
...
- External modification of a clean open file can be reloaded.
- External modification of a dirty open file enters conflict state.
- Autosave pauses for conflicted documents.
- User can save local dirty content as a new file when original path is unsafe.
