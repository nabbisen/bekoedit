# RFC-009: Application State Store and Command/Event Model
...
Defines the application-level state model, command routing, event flow, and separation between UI projections and core state.

---

## 2. Motivation

- As editing modes grow, unmanaged state can create inconsistent UI and accidental mutation paths.
- A command/event model makes testing and safety review easier.

---

## 3. Goals

- Centralize app state transitions.
- Represent user actions as commands.
- Represent core updates as events.
- Keep UI projections derived and disposable.
...
- Implement a distributed event log.
- Persist every UI event.
- Adopt a complex actor framework before needed.
- Expose internal events to plugins.
...
- The user should experience consistent state: selected file, editor mode, save state, explorer state, and banners must not contradict each other.
...
struct AppState {
    ...
    open_documents: DocumentRegistry,
    active_document: Option<DocumentId>,
    ui_layout: LayoutState,
...
enum AppCommand {
    ...
    File(FileOperationCommand),
    Ui(UiCommand),
}
```

Commands are requests. Events record accepted state changes.

---

## 8. Internal Design Notes

- Use explicit reducer/update functions for state transitions.
- Do not let UI components mutate global state directly.
- Record error events in a user-visible notification model.
- Keep document core independent of Dioxus component lifecycle.
...
- Each user-visible action maps to a command.
- Core state changes can be unit-tested.
- Invalid commands are rejected safely.
- UI can be reconstructed from AppState projections.
