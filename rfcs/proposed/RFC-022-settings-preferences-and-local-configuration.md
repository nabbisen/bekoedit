...
Defines local user preferences such as theme, autosave, editor font size, ignored directories, and recent workspaces.

---

## 2. Motivation

- The app needs simple persistent preferences without becoming a configuration-heavy IDE.
- Settings influence save safety and editing comfort.

---

## 3. Goals

- Store local app preferences.
- Expose a minimal settings page.
- Support autosave toggle/debounce, theme, font size, explorer ignore patterns, and reopen last workspace.
- Keep settings separate from Markdown documents.

---

## 4. Non-Goals

- Implement cloud-synced settings.
- Create workspace-specific settings in MVP unless necessary.
- Expose hidden advanced parser toggles to normal users.
...
- Settings page sections: General, Editor, Files, Accessibility, About.
- Changes should apply immediately where safe.
- Risky settings should have explanatory text.
...
struct UserSettings {
    ...
    reopen_last_workspace: bool,
    explorer_ignore_patterns: Vec<String>,
}
```

Settings live in OS-appropriate app config storage.

---

## 8. Internal Design Notes

- Load settings before opening the main window where feasible.
- Validate settings and recover from corrupt config with defaults.
- Do not write settings into the workspace by default.
...
- Corrupt settings file does not prevent app launch.
- Autosave can be disabled.
- Ignore patterns affect explorer refresh.
