...
pub fn FormBlockView(
    ...
    let (group_name, body) = match display {
        // ── Heading ──────────────────────────────────────────────────────────
        FormBlockDisplay::Heading {
            ...
            tr(lang, "block.kind.heading").to_string(),
            rsx! {
                div { class: "block-row",
                    select {
        ...
        // ── Paragraph ────────────────────────────────────────────────────────
        FormBlockDisplay::Paragraph { text } => (
            tr(lang, "block.kind.paragraph").to_string(),
            rsx! {
        ...
        // ── Blockquote ───────────────────────────────────────────────────────
        FormBlockDisplay::Blockquote { text } => (
            tr(lang, "block.kind.blockquote").to_string(),
            rsx! {
                InlineToolbar { field_id: field_id.clone(), block_id, revision, lang }
                textarea {
                    id: "{field_id}",
        ...
        // ── List ─────────────────────────────────────────────────────────────
        FormBlockDisplay::List { ordered, items } => (
            tr(lang, "block.kind.list").to_string(),
            rsx! {
        ...
        FormBlockDisplay::Code { language, code } => {
            ...
            (
                tr(lang, "block.kind.code").to_string(),
                rsx! {
        ...
        // ── Horizontal rule ──────────────────────────────────────────────────
        FormBlockDisplay::HorizontalRule => (
            tr(lang, "block.kind.horizontal_rule").to_string(),
            rsx! { hr {} },
        ),
        ...
        // ── Image card (RFC-028) ──────────────────────────────────────────────
        FormBlockDisplay::Image { alt, src } => (
            tr(lang, "block.kind.image").to_string(),
            rsx! {
        ...
        FormBlockDisplay::RawIsland {
            ...
            // States both that this is a raw region and why (handoff §5.2):
            // reuses the existing island.* reason strings and island.hint
            // rather than inventing new reason text.
            let group_name = format!("{}: {}", tr(lang, &label_key), tr(lang, "island.hint"));
            (
                group_name,
                rsx! {
