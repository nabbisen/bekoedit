...
Defines the in-memory document session, canonical raw text, revisions, encoding policy, and projection ownership.

---

## 2. Motivation

- Every editing feature depends on a safe document model.
- The canonical source model prevents rich-text or preview state from becoming durable state.

---

## 3. Goals

- Represent each open Markdown file as a document session.
- Store canonical raw text in Rust memory.
- Track revisions, dirty state, file metadata, and derived projections.
- Preserve line ending style and reasonable encoding behavior.

---

## 4. Non-Goals

- Implement collaborative multi-writer sessions.
- Support arbitrary binary encodings as editable text.
- Store durable document state in a database.
- Let Form Mode own the document model.
...
- Opening a file loads it into the editor area.
- The status bar shows clean/dirty/saving/saved/error state.
- If loading fails, the user sees an actionable error rather than an empty editor.
...
struct DocumentSession {
    ...
    base_disk_fingerprint: FileFingerprint,
...
enum LineEnding { Lf, Crlf, MixedNormalizeOnWrite }
enum EncodingPolicy { Utf8, Utf8WithBomDetected, UnsupportedButReadLosslessBytesBlocked }
```

The document session is the only place where durable text mutation occurs.

---

## 8. Internal Design Notes

- Read files as UTF-8 by default, with explicit error handling for invalid input.
- Preserve original line endings where practical.
- Increment revision after every accepted mutation.
- Rebuild projections after mutation.
- Keep session operations testable without GUI.
...
- Opening a UTF-8 Markdown file creates a document session.
- Editing canonical text changes revision and dirty state.
- Projection data can be rebuilt from canonical text.
- Invalid or unsupported encoding is reported safely.
