...
//! §2.3/§4.3: RFC-041's `PhaseMachine::validate` now surfaces a terminal
//! failure's own reason through every structural rejection -- the same
//! property task 023 review (root-cause-fixed) §2 found missing and fixed
//! for `trusted_click.rs`'s own validator, generalised here via the shared
//! `transport::reject_with_reason` helper.

use crate::webview_smoke::protocol::{PhaseMachine, SmokePhase};
...
fn a_terminal_failure_s_own_reason_survives_a_structural_pin_mismatch() {
    let machine = PhaseMachine::for_phase(SmokePhase::Editor);
    let message = PhaseMessage {
        ...
        result: Some(DriverResult {
            ...
            marker: "RFC041_WEBVIEW_SMOKE_MARKER".into(),
            milestones: Vec::new(),
